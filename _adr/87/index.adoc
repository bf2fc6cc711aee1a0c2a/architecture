---
num: 87 
title: Process for requesting and naming metrics exposed in the KAS Fleet Manager metrics API
status: "Draft"
authors:
  - "JameelB"
tags:
...
|===
|Category | Description | Sub-component (to be specified in the metric name)

|Instance
|General Kafka metrics not specific to any other sub-components.
|instance
...
|topic (Even though the topic metrics already have a 'topic' label, including this improves the clarity of the metric name)
|===

Examples of sub-component usage in the metric name:

* For *Instance* metrics, the metric prefix would be `kas_instance_`.
* For *Broker* metrics, the metric prefix would be `kas_broker_`.
* For *Topic* metrics, the metric prefix would be `kas_topic_`.
...
For Kafka metrics, we can use `instance_name` and `instance_id` to identify the Kafka instance the metric belongs to.

#### Cluster Metrics
|===
|Old Metric Name | Revised Metric Name | Type | Old Labels | Revised Labels | Description

|kafka_instance_max_message_size_limit
|kas_instance_message_size_limit_bytes
|Gauge
|Not available
|instance_name, instance_id
|Maximum message size, in bytes, supported by the instance.

|kafka_instance_partition_limit
|kas_instance_partition_limit
|Gauge
|Not available
|instance_name, instance_id
...
|Not available
|instance_name, instance_id
|Number of incoming bytes per second in the last five minutes per instance

|kafka_namespace:haproxy_server_bytes_out_total:rate5m
|kas_instance_outgoing_bytes_rate5m
|Gauge
|Not available
|instance_name, instance_id
|Number of outgoing bytes per second in the last five minutes per instance

|kafka_topic:kafka_topic_partitions:count
|kas_instance_topic_total
|Gauge
|Not available
|instance_name (source label: strimzi_io_cluster), instance_id
|Total number of topics, excluding system topics, per instance

|kafka_topic:kafka_topic_partitions:sum
|kas_instance_topic_partition_total
|Gauge
|Not available
|instance_name (source label: strimzi_io_cluster), instance_id
|Total number of topics across all topics, excluding system topics, per instance

|consumergroup:kafka_consumergroup_members:count
|kas_instance_consumer_group_total
|Gauge
|Not available
|instance_name (source label: strimzi_io_cluster), instance_id
|Total number of consumer groups, excluding system consumer groups, per instance
...
|Not available
|instance_name, instance_id
|Total number of connections, excluding system connections apart from canary, per instance

|kafka_namespace:kafka_server_socket_server_metrics_connection_creation_rate:sum
|kas_instance_connection_creation_rate
|Gauge
|Not available
|instance_name, instance_id
|Rate of connections established per second, excluding system connections apart from canary, per instance

|kafka_instance_spec_brokers_desired_count
|kas_instance_desired_broker_total
|Gauge
|Not available
|instance_name, instance_id
|Number of desired brokers per instance
...
|kas_broker_volume_available_bytes
|Gauge
|persistentvolumeclaim
|volume (source label: persistentvolumeclaim), instance_name, instance_id
|Number of available disk space, in bytes, per broker

|kafka_broker_quota_totalstorageusedbytes
|kas_broker_storage_used_bytes
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id
|Number of storage used by partitions, in bytes, per broker

|kafka_broker_quota_hardlimitbytes
|kas_broker_storage_hard_limit_bytes
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id
|The maximum number of storage, in bytes, per broker before producers are blocked.

|kafka_broker_quota_softlimitbytes
|kas_broker_storage_soft_limit_bytes
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id
|The maximum number of storage, in bytes, per broker before producers are progressively throttled.

|kafka_broker_client_quota_limit
|kas_broker_client_quota_limit
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, quota_type, strimzi_io_cluster
|broker_id, quota_type, instance_name (source label: strimzi_io_cluster), instance_id
|Request, produce and fetch limits per broker

|kafka_instance_connection_limit
|kas_broker_connection_limit
|Gauge
|broker_id
|broker_id, instance_name, instance_id
...
|broker_id
|broker_id, instance_name, instance_id
|Maximum connection rate per second per broker

|kafka_controller_kafkacontroller_offline_partitions_count
|kas_broker_offline_partition_total
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id
|Number of partitions that are currently offline per broker

|kafka_controller_kafkacontroller_global_partition_count
|kas_broker_global_partition_total
|Gauge
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id
|Number of partitions across all topics in the cluster, including system topics, per broker
...
|kas_topic_incoming_messages_total
|Counter
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), instance_id, topic
|Total number of incoming messages per topic per broker

|kafka_topic:kafka_server_brokertopicmetrics_messages_in_total:rate5m
|kas_topic_incoming_messages_rate5m
|Gauge
|statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), topic, instance_id
|Rate of incoming messages per topic, excluding system topics, per broker

|kafka_server_brokertopicmetrics_bytes_in_total
|kas_topic_incoming_bytes_total
|Counter
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), topic, instance_id
|Total number of incoming bytes per topic per broker

|kafka_topic:kafka_server_brokertopicmetrics_bytes_in_total:rate5m
|kas_topic_incoming_bytes_rate5m
|Gauge
|statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), topic, instance_id
|Rate of incoming bytes per topic, excluding system topics, per broker

|kafka_server_brokertopicmetrics_bytes_out_total
|kas_topic_outgoing_bytes_total
|Counter
|broker_id, statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), topic, instance_id
|Total number of outgoing bytes per topic per broker

|kafka_topic:kafka_server_brokertopicmetrics_bytes_out_total:rate5m
|kas_topic_outgoing_bytes_rate5m
|Gauge
|statefulset_kubernetes_io_pod_name, strimzi_io_cluster, topic
|broker_id, instance_name (source label: strimzi_io_cluster), topic, instance_id
|Rate of outgoing bytes per topic, excluding system topics, per broker

|kafka_topic:kafka_log_log_size:sum
|kas_topic_log_size_bytes
|Gauge
|topic
|topic, instance_name, instance_id
|Log size, in bytes, per topic

|===

### Metrics to be removed
The following metrics will be deprecated and removed from the KAS Fleet Manager metrics API.

* *kubelet_volume_stats_used_bytes*: Duplicates `kafka_broker_quota_totalstorageusedbytes`.
...
*** (Optional) Category/Sub-component which this new metric belongs to is identified.
...
### Configuring the metric and label names

The https://github.com/bf2fc6cc711aee1a0c2a/observability-resources-mk[observability-resources-mk] repository contains all of our customer facing recording rules and remote-write configuration.
All customer facing metric aggregation and re-naming of the metric and label name should be done here.
This will allow KAS Fleet Manager to continue to act as a proxy only where no additional aggregation will be required apart from filtering by the user's Kafka instance.

Each customer facing metric must be included in the https://github.com/bf2fc6cc711aee1a0c2a/observability-resources-mk/blob/main/resources/prometheus/user-facing-recording-rules.yaml[user-facing-recording-rules.yaml].

* *expr*: Expression must include label filtering and re-naming. 
Prometheus' https://prometheus.io/docs/prometheus/latest/querying/functions/#label_replace[label_replace()] can be used to re-name the labels.
See example usage of this function in the https://github.com/bf2fc6cc711aee1a0c2a/observability-resources-mk/blob/main/resources/prometheus/billing-recording-rules.yaml[billing metrics].

* *record*: Must have the value of the new metric name which adheres to the naming policy as described above.

These recording rules can then be included in the Prometheus https://github.com/bf2fc6cc711aee1a0c2a/observability-resources-mk/blob/main/resources/prometheus/remote-write.yaml[remote-write] configuration to ensure they're pushed to Observatorium.
