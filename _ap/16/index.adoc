...
*<service-name>_<optional:sub-component>_<descriptor>_<optional:base-unit>_<optional:type|aggregate>*

* *service-name*: The metric name should be prefixed with the service name (e.g. `kas`)
* *sub-component* (optional): The sub-component of the service that the metric measures.
This should be used for logically categorizing the metrics. 
The different sub-components of the service must be agreed on by the team (e.g. `instance`, `broker`, `topic`).
